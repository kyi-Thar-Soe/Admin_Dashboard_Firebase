...
export default function ProductList() {
  ...
  return (
    <>
      ...
      >
        <Box sx={style}>
          <EditForm handleEditClose={handleEditClose} formid={formid}/>
        </Box>
      </Modal>
    </div>
    {rows.length > 0 && (
    <Paper sx={{ width: '100%', overflow: 'hidden' }}>
        <Typography gutterBottom variant='h5' component="div" sx={{padding: "20px 20px 0px 20px"}}>
                Product List
        </Typography>
        <Divider style={{background: "rgba(224, 224, 224, 1)"}}/>
        <Box height={10} />
          <Stack direction="row" spacing={2} className="my-2 mb-2 ms-3 me-3">
            <Autocomplete
              disablePortal
              id="combo-box-demo"
              options={rows}
              ...
              .map((row,index) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                        ...
                        <TableCell key={`${row.id}-price`} align="left"> {row.price}</TableCell>
